...
def detect_openvpn_dir():
    candidates = ["/etc/openvpn/server", "/etc/openvpn"]
    for d in candidates:
        if os.path.isdir(d) and os.path.isfile(os.path.join(d, "server.conf")):
            return d
    for d in candidates:
        if os.path.isdir(d):
            return d
    return "/etc/openvpn/server"

OPENVPN_DIR = detect_openvpn_dir()
SERVER_CONF = os.path.join(OPENVPN_DIR, "server.conf")
CCD_DIR = os.path.join(OPENVPN_DIR, "ccd")
STATUS_LOG_DEFAULT = os.path.join(OPENVPN_DIR, "status.log")
# ==============================================
...
def get_main_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("🚨 Тревога ON", callback_data='block_alert')],
...
def main():
    ...
    import asyncio
    loop = asyncio.get_event_loop()# -*- coding: utf-8 -*-
"""
OpenVPN Telegram Monitor Bot
(Изменения 2025-10-01):
  * Натуральная сортировка имён клиентов (1,2,3,...10,11 вместо 1,10,11,2,...)
